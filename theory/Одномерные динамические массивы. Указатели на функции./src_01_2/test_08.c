...
{
    *n = 0;      // ? это значение меняется, даже если была ошибка

    //
    // определить количество элементов (возможно, это сложный процесс)
    //
...
int get_array_2(double **data, int *n)
{
    ...
    *n = 0;             // ? это значение меняется, даже если была ошибка
    *data = NULL;       // ? это значение меняется, даже если была ошибка
